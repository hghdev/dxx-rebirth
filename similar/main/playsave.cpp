...
void write_player_file()
{
	...
	if ((PHYSFS_writeBytes( file, PlayerCfg.HighestLevels.data(), sizeof(hli) * PlayerCfg.NHighestLevels) != PlayerCfg.NHighestLevels)) {
		errno_ret = errno;
		return;
	}

	if (PHYSFS_writeBytes( file, saved_games.data(),sizeof(saved_games)) != 1) {
		errno_ret = errno;
		return;
	}

	range_for (auto &i, PlayerCfg.NetworkMessageMacro)
		if (PHYSFS_writeBytes(file, i.data(), i.size()) != 1) {
		errno_ret = errno;
		return;
	}
	...
	//write kconfig info
	{
		if (PHYSFS_writeBytes(file, PlayerCfg.KeySettings.Keyboard.data(), sizeof(PlayerCfg.KeySettings.Keyboard)) != 1)
			errno_ret=errno;
#if DXX_MAX_JOYSTICKS
		auto &KeySettingsJoystick = PlayerCfg.KeySettings.Joystick;
#else
...
#endif
		if (PHYSFS_writeBytes(file, KeySettingsJoystick.data(), sizeof(KeySettingsJoystick)) != 1)
			errno_ret=errno;
		for (unsigned i = 0; i < MAX_CONTROLS*3; i++)
			if (PHYSFS_writeBytes(file, "0", sizeof(ubyte)) != 1) // Skip obsolete Flightstick/Thrustmaster/Gravis map fields
				errno_ret=errno;
		if (PHYSFS_writeBytes(file, PlayerCfg.KeySettings.Mouse.data(), sizeof(PlayerCfg.KeySettings.Mouse)) != 1)
			errno_ret=errno;
		{
			std::array<uint8_t, MAX_CONTROLS> cyberman{};
			if (PHYSFS_writeBytes(file, cyberman.data(), cyberman.size()) != 1)	// Skip obsolete Cyberman map field
				errno_ret=errno;
		}
		...
		if(errno_ret == EZERO)
		{
			...
			if (PHYSFS_writeBytes( file,  &PlayerCfg.ControlType, sizeof(ubyte) )!=1)
				errno_ret=errno;
			else if (PHYSFS_writeBytes( file, &old_avg_joy_sensitivity, sizeof(ubyte) )!=1)
				errno_ret=errno;
		}
	}
	...
	if ((PHYSFS_writeBytes(file, PlayerCfg.HighestLevels, sizeof(hli) * PlayerCfg.NHighestLevels) != PlayerCfg.NHighestLevels))
	...
	//write kconfig info
	{
		...
		if (PHYSFS_writeBytes(file, PlayerCfg.KeySettings.Keyboard, sizeof(PlayerCfg.KeySettings.Keyboard)) != 1)
			goto write_player_file_failed;
#if DXX_MAX_JOYSTICKS
		auto &KeySettingsJoystick = PlayerCfg.KeySettings.Joystick;
#else
		const std::array<uint8_t, MAX_CONTROLS> KeySettingsJoystick{};
#endif
		if (PHYSFS_writeBytes(file, KeySettingsJoystick, sizeof(KeySettingsJoystick)) != 1)
			goto write_player_file_failed;
		for (unsigned i = 0; i < MAX_CONTROLS*3; i++)
			if (PHYSFS_writeBytes(file, "0", sizeof(ubyte)) != 1) // Skip obsolete Flightstick/Thrustmaster/Gravis map fields
				goto write_player_file_failed;
		if (PHYSFS_writeBytes(file, PlayerCfg.KeySettings.Mouse, sizeof(PlayerCfg.KeySettings.Mouse)) != 1)
			goto write_player_file_failed;
		for (unsigned i = 0; i < MAX_CONTROLS*2; i++)
			if (PHYSFS_writeBytes(file, "0", sizeof(ubyte)) != 1) // Skip obsolete Cyberman/Winjoy map fields
				goto write_player_file_failed;
		if (PHYSFS_writeBytes(file, &control_type_dos, sizeof(ubyte)) != 1)
			goto write_player_file_failed;
		ubyte control_type_win = 0;
		if (PHYSFS_writeBytes(file, &control_type_win, sizeof(ubyte)) != 1)
			goto write_player_file_failed;
		if (PHYSFS_writeBytes(file, &old_avg_joy_sensitivity, sizeof(ubyte)) != 1)
			goto write_player_file_failed;

		range_for (const unsigned i, xrange(11u))
		{
			PHYSFS_writeBytes(file, &PlayerCfg.PrimaryOrder[i], sizeof(ubyte));
			PHYSFS_writeBytes(file, &PlayerCfg.SecondaryOrder[i], sizeof(ubyte));
		}
