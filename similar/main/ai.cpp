...
int ai_save_state(PHYSFS_File *fp)
{
	...
	PHYSFS_writeBytes(fp, &Ai_initialized, sizeof(int));
	PHYSFS_writeBytes(fp, &Overall_agitation, sizeof(int));
	{
		ai_local_rw zero{};
		range_for (const auto &i, Objects)
		{
			ai_local_rw ail_rw;
			PHYSFS_writeBytes(fp, i.type == OBJ_ROBOT ? (state_ai_local_to_ai_local_rw(&i.ctype.ai_info.ail, &ail_rw), &ail_rw) : &zero, sizeof(ail_rw));
		}
	}
	PHYSFSX_serialize_write(fp, Point_segs);
	//PHYSFS_writeBytes(fp, Ai_cloak_info, sizeof(ai_cloak_info) * MAX_AI_CLOAK_INFO);
	range_for (auto &i, Ai_cloak_info)
	{
		ai_cloak_info_rw aic_rw;
		state_ai_cloak_info_to_ai_cloak_info_rw(&i, &aic_rw);
		PHYSFS_writeBytes(fp, &aic_rw, sizeof(aic_rw));
	}
	const auto gametime{GameTime64};
	{
		const auto Boss_cloak_start_time = BossUniqueState.Boss_cloak_start_time;
	...
	}
	{
		...
		PHYSFS_writeBytes(fp, &Boss_teleport_interval, sizeof(fix));
		const fix Boss_cloak_interval = LevelSharedBossState.Boss_cloak_interval;
		PHYSFS_writeBytes(fp, &Boss_cloak_interval, sizeof(fix));
	}
	PHYSFS_writeBytes(fp, &Boss_cloak_duration, sizeof(fix));
	tmptime32 = build_savegametime_from_gametime(gametime, BossUniqueState.Last_gate_time);
	PHYSFS_writeBytes(fp, &tmptime32, sizeof(fix));
	PHYSFS_writeBytes(fp, &GameUniqueState.Boss_gate_interval, sizeof(fix));
	{
	...
	PHYSFS_writeBytes(fp, &boss_dying, sizeof(int));
	}
	const int boss_dying_sound_playing = BossUniqueState.Boss_dying_sound_playing;
	PHYSFS_writeBytes(fp, &boss_dying_sound_playing, sizeof(int));
#if defined(DXX_BUILD_DESCENT_I)
	const int boss_hit_this_frame = BossUniqueState.Boss_hit_this_frame;
	PHYSFS_writeBytes(fp, &boss_hit_this_frame, sizeof(int));
	const int Boss_been_hit = 0;
	PHYSFS_writeBytes(fp, &Boss_been_hit, sizeof(int));
#elif defined(DXX_BUILD_DESCENT_II)
	...
	PHYSFS_writeBytes(fp, &Escort_goal_object, sizeof(Escort_goal_object));
	}
	{
		const uint32_t Escort_special_goal = BuddyState.Escort_special_goal;
	PHYSFS_writeBytes(fp, &Escort_special_goal, sizeof(Escort_special_goal));
	}
	{
		...
		PHYSFS_writeBytes(fp, &egi, sizeof(int));
	}
	{
		auto &Stolen_items = LevelUniqueObjectState.ThiefState.Stolen_items;
		PHYSFS_writeBytes(fp, &Stolen_items, sizeof(Stolen_items[0]) * Stolen_items.size());
	}

	{
		int temp;
		temp = Point_segs_free_ptr - Point_segs;
		PHYSFS_writeBytes(fp, &temp, sizeof(int));
	}

	unsigned Num_boss_teleport_segs = Boss_teleport_segs.size();
	PHYSFS_writeBytes(fp, &Num_boss_teleport_segs, sizeof(Num_boss_teleport_segs));
	unsigned Num_boss_gate_segs = Boss_gate_segs.size();
	PHYSFS_writeBytes(fp, &Num_boss_gate_segs, sizeof(Num_boss_gate_segs));

	if (Num_boss_gate_segs)
		PHYSFS_writeBytes(fp, &Boss_gate_segs[0], sizeof(Boss_gate_segs[0]) * Num_boss_gate_segs);

	if (Num_boss_teleport_segs)
		PHYSFS_writeBytes(fp, &Boss_teleport_segs[0], sizeof(Boss_teleport_segs[0]) * Num_boss_teleport_segs);
#endif
